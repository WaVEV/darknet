...
public:
	...
	bp::list detect_object(bp::str img_data, int img_width, int img_height, int img_channel){
		// preprocess input image
                assert(img_channel == 3 && img_width == WIDTH_IMAGE && img_height == HEIGHT_IMAGE);
		...
		assert(img_channel == 3);
                image im; im.h = img_height; im.w = img_width; im.c = img_channel;im.data = buff_img;
		int cnt = img_height * img_width;
		for (int i = 0; i < cnt; ++i){
		    im.data[i] = (float)data[i + 2 * cnt] / 255.;
		}
                for (int i = 0; i < cnt; ++i){
                    im.data[i+cnt] = (float)data[i + cnt] / 255.;
                }
                for (int i = 0; i < cnt; ++i){
                    im.data[i+2*cnt] = (float)data[i] / 255.;
                }
			//rgbgr_image(im);
		float *X = im.data;
		...
		draw_detections_bbox(im, layer.side*layer.side*layer.n, thresh, boxes, probs, voc_class_names, 0, 1, bboxes);
